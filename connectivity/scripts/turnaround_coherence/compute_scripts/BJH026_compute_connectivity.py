...
for subject in subject_list:
    ...
    elec_list = pd.Series(insula_list + amyg_list + ofc_list + cing_list + dlpfc_list)

    # Identify duplicated elements
    duplicated_list = elec_list[elec_list.duplicated()].tolist()

    # ROI lists
    roi_lists = [insula_list, amyg_list, ofc_list, cing_list, dlpfc_list]
    ...
    for perm in range(0, permutations):
        ...
        ## combine ##
        last_away_roi = last_away_insula.add_channels([last_away_amyg, last_away_ofc, last_away_cing, last_away_dlpfc])
