...
for subject in subject_list:
    ...
    for perm in range(0, permutations):
        ...
        insula_pairs = [(ins_roi, roi) for roi in hc_list + amyg_list + ofc_list + dlpfc_list for ins_roi in insula_list]
        # Find indices in ch_names matching the first element of each pair
        first_pair_indices = [idx for pair in insula_pairs 
                                        for idx, roi in enumerate(last_away_roi.info['ch_names']) 
                                        if roi == pair[0]]
        second_pair_indices = [idx for pair in insula_pairs 
                                        for idx, roi in enumerate(last_away_roi.info['ch_names']) 
                                        if roi == pair[1]] 
        ...
        if perm % 10 == 0:
            np.save(f'/global/scratch/users/bstavel/pacman/across_subject_analyses/ieeg/connectivity/perms/{subject}_imcoh_insula_perm.npy', imcoh_permutations)
            np.save(f'/global/scratch/users/bstavel/pacman/across_subject_analyses/ieeg/connectivity/perms/{subject}_ppc_insula_perm.npy', ppc_permutations)
            np.save(f'/global/scratch/users/bstavel/pacman/across_subject_analyses/ieeg/connectivity/perms/{subject}_pli_insula_perm.npy', pli_permutations)

    # final save
    np.save(f'/global/scratch/users/bstavel/pacman/across_subject_analyses/ieeg/connectivity/perms/{subject}_imcoh_insula_perm.npy', imcoh_permutations)
    np.save(f'/global/scratch/users/bstavel/pacman/across_subject_analyses/ieeg/connectivity/perms/{subject}_ppc_insula_perm.npy', ppc_permutations)
    np.save(f'/global/scratch/users/bstavel/pacman/across_subject_analyses/ieeg/connectivity/perms/{subject}_pli_insula_perm.npy', pli_permutations)
    np.save(f'/global/scratch/users/bstavel/pacman/across_subject_analyses/ieeg/connectivity/perms/{subject}_insula_pairs.npy', (first_pair_indices, second_pair_indices))
