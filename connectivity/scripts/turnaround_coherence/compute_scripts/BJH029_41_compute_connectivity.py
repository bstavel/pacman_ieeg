...
## No Cingulate, No dlpfc

## Prep paths ##
subject_list = ['BJH029', 'BJH041']
...
for subject in subject_list:
    ...
    elec_list = pd.Series(hc_list + ofc_list + amyg_list + insula_list)

    # Identify duplicated elements
    duplicated_list = elec_list[elec_list.duplicated()].tolist()

    # ROI lists
    roi_lists = [hc_list, ofc_list, amyg_list, insula_list]
    ...
    amyg_list = roi_lists[2]
    insula_list = roi_lists[3]
    roi_lists = [hc_list, ofc_list, amyg_list, insula_list]
    ...
    for perm in range(0, permutations):
        ...
        ## combine ##
        last_away_roi = last_away_hc.add_channels([last_away_ofc, last_away_amyg, last_away_insula])
        ...
        if perm % 10 == 0:
            np.save(f'/global/scratch/users/bstavel/pacman/connectivity/ieeg/perms/{subject}_imcoh_perm.npy', imcoh_permutations)
            np.save(f'/global/scratch/users/bstavel/pacman/connectivity/ieeg/perms/{subject}_ppc_perm.npy', ppc_permutations)
            np.save(f'/global/scratch/users/bstavel/pacman/connectivity/ieeg/perms/{subject}_pli_perm.npy', pli_permutations)

    # final save
    np.save(f'/global/scratch/users/bstavel/pacman/connectivity/ieeg/perms/{subject}_imcoh_perm.npy', imcoh_permutations)
    np.save(f'/global/scratch/users/bstavel/pacman/connectivity/ieeg/perms/{subject}_ppc_perm.npy', ppc_permutations)
    np.save(f'/global/scratch/users/bstavel/pacman/connectivity/ieeg/perms/{subject}_pli_perm.npy', pli_permutations)
    np.save(f'/global/scratch/users/bstavel/pacman/connectivity/ieeg/perms/{subject}_pairs.npy', (first_pair_indices, second_pair_indices))
